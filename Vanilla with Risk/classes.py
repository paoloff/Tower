...
class Portfolio():
    ...
    def update(self, open, close, risk, signal, risk_aversion=.1):
        ...
        total_risk = risk_aversion*np.sum(np.sqrt(risk))
        return (close_nav - open_nav)/len(self.assets) - total_risk, close, self.allocations, self.cash, close_nav, total_risk
        #return np.random.rand(), close, self.allocations, self.cash, close_nav, total_risk

    def risk(self, y_data):
        n_data = len(y_data[0])
        x_data = np.arange(0,n_data)
        X = np.ones((n_data,2))
        X[:,1] = x_data
        H = np.matmul(X,np.matmul(np.linalg.inv(np.matmul(np.transpose(X), X)),np.transpose(X)))
        var = np.zeros(len(y_data))
        for i in range(len(y_data)): var[i] = (np.inner(y_data[i], y_data[i]) - np.matmul(np.matmul(np.transpose(y_data[i]), H), y_data[i]))
        return var/n_data
...
class MarketSimulator(gym.Env):
    def __init__(self, cash, assets, allocations, interest_rate, interval, period, multiple, steps_per_episode):
        self.time_step = 0
    ...
    def step(self, action):
        ...
        risk = self.portfolio.risk([np.array(self.open[i][(self.time_step-self.multiple):self.time_step])for i in range(self.n_assets)])
        #risk = 0
        self.time_step += 1
        return self.portfolio.update(open_prices, close_prices, risk, action)
    ...
    def reset(self, cash, assets, allocations):
        ...
        self.time_step = random.randint(self.multiple+1, self.data_length - self.max_number_of_steps - 1)
        #self.time_step = 500


class QNN(torch.nn.Module):
    def __init__(self, n_assets, n1=3, n2=200, lr=0.05, momentum=0.9):
...
class Learner():
    def __init__(self, assets, epsilon=0.1, batch_size=20, gamma=0.9, experience_buffer_size=50, tau=10):
    ...
    def epsilon_greedy(self, state):
        self.total_steps += 1
        epsilon = self.epsilon*(np.exp(-self.total_steps/180))
        #print(epsilon)
